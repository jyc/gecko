...
static bool
RebuildAllStyleData(Document* aDocument) {
  nsPresContext* pcx;
  if ((pcx = aDocument->GetPresContext())) {
    pcx->PostRebuildAllStyleDataEvent(nsChangeHint(0), RestyleHint::RestyleSubtree());
    return true;
  } else {
    return false;
  }
...
static void
HandlePropertyRegistrationResult(Document* aDocument,
                                 mozilla::PropertyRegistrationResult aResult,
                      ...
                      mozilla::ErrorResult& aRv)
{
  ...
  HandlePropertyRegistrationResult(
    ...
    document->StyleSetForPresShellOrMediaQueryEvaluation()
            ->RegisterProperty(aDescriptor.mName,
                               aDescriptor.mSyntax,
                               aDescriptor.mInherits,
                               aDescriptor.mInitialValue.WasPassed(),
                               maybeInitialValue),
    aRv);
}
