...
pub fn parse_name(s: &str) -> Result<&str, ()> {
    ...
    match input.expect_ident() {
        Ok(_) if s.starts_with("--") => match input.expect_exhausted() {
            Ok(()) => Ok(&s[2..]),
            Err(_) => Err(()),
        },
        _ => Err(()),
...
pub struct VariableValue {
    ...
    /// HashSet; we wrap in an Option so we don't have to create a new HashSet
    /// every time we are applying an interpolated custom property.
    pub references: Option<Box<[Name]>>,
...
impl VariableValue {
    ...
    pub fn parse<'i, 't>(
        ...
        Ok(Arc::new(VariableValue {
            ...
            references: Some(custom_property_references),
