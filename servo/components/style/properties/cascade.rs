    ...
    I: Iterator<Item = (&'a PropertyDeclaration, CascadeLevel)>,
{
    ...
    for (declaration, cascade_level) in iter_declarations() {
        ...
        match declaration {
            PropertyDeclaration::WithVariables(VariableDeclaration { id: LonghandId::FontSize, ref value }) => {
                ...
                font_size_references = Some(references.custom_property_references.into_iter().collect());
            },
    ...
    let (custom_properties, only_early_custom_properties, font_size_cyclical) = custom_properties_builder.build_early(
        Some(&context),
        font_size_references.as_ref().map(Vec::as_slice).unwrap_or(&[]),
    );
    ...
    if context.builder.modified_reset() || using_cached_reset_properties ||
       context.builder.custom_properties.as_ref().map(|p| p.has_uninherited).unwrap_or(false) {
