...
use crate::custom_properties::{
    self, substitute_references_in_value, CssEnvironment, EmptySubstitutionMap, ExtraData, Name,
    ResolutionMap, SubstitutionMap, TokenStream, VariableValue,
};
...
pub fn register_property(
    ...
    let computed_initial_value = initial_value.map(|initial_value| {
        let value = computed::Context::for_media_query_evaluation(
            device,
...
fn detect_ems<'i, 'tt>(input: &mut Parser<'i, 'tt>) -> Result<bool, cssparser::ParseError<'i, ()>> {
    while !input.is_exhausted() {
        ...
        match token {
            Token::Function(_) => {
                input.parse_nested_block(detect_ems)?;
            },
    ...
    fn new_early(
        ...
        for (name, value) in custom_properties.iter() {
            ...
            let syntax_has_length_component =
                registration.map_or(false, |r| r.syntax.has_length_component());
            ...
            let mut input = Parser::new(&mut input);
            if detect_ems(&mut input).unwrap_or(false) {
                font_size_dependent
    ...
    fn has_references(&self, name: &PVName) -> bool {
        match *name {
            PVName::Variable(ref name) => {
                ...
                has_references ||
                    self.font_size_dependent
                        .as_ref()
                        .map_or(false, |d| d.contains(name))
            },
            PVName::FontSize => self.font_size_references.map_or(false, |r| r.len() > 0),
        }
    }
                ...
                let substituted = {
                    ...
                    if !value.has_references() {
                        ...
                        match (&value.references, &mut self.font_size_dependent) {
                            (references, &mut Some(ref mut font_size_dependent)) => {
...
impl<'a> CustomPropertiesBuilder<'a> {
        ...
        match (value, inherits) {
            (&CustomDeclarationValue::Value(ref unparsed_value), _) => {
                ...
                let should_substitute_environment_references =
                    !self.may_require_resolution && unparsed_value.references_environment;
    ...
    fn value_may_affect_style(&self, name: &Name, value: &CustomDeclarationValue) -> bool {
        ...
        match (value, inherits) {
            ...
            // value.
            (&CustomDeclarationValue::CSSWideKeyword(CSSWideKeyword::Inherit), true) |
            (&CustomDeclarationValue::CSSWideKeyword(CSSWideKeyword::Unset), true) => false,
            ...
            (&CustomDeclarationValue::CSSWideKeyword(CSSWideKeyword::Initial), false) |
            (&CustomDeclarationValue::CSSWideKeyword(CSSWideKeyword::Unset), false) => false,
            ...
            (&CustomDeclarationValue::Value(ref unparsed_value), false) => {
                ...
                if let Some(&(ref _value, ref css)) =
                    registration.and_then(|r| r.initial_value.as_ref())
                {
    ...
    pub fn build_early(
        ...
        let mut custom_properties = match self.result_properties.take() {
            Some(ps) => ps,
            None => {
                let parent_has_uninherited =
                    self.parent_properties.map_or(false, |x| x.has_uninherited);
